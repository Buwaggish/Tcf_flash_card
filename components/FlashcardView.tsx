...
export const FlashcardView: React.FC<FlashcardViewProps> = ({ cards, title, onBack }) => {
  ...
  const handlePlaySequence = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      for (const word of words) {
        // Check if component is still mounted/valid implicitly by loop continuation
        // Ideally we'd use a ref to check if unmounted, but standard usage implies linear flow
        ...
        <button 
          onClick={() => {
              ...
              if (!isFlipped && !isPlaying) {
                  // Optional: Auto-play when flipping to back
                  // handlePlayAudio();
              }
          }}
